...
\textbf{Tertiary Metrics (Cost, Robustness, and Explainability):} To rigorously address RQ2, we redefine three core metrics:
\begin{enumerate}
...
To validate our approach, we employed the \textbf{Hypervolume (HV)} indicator, calculated explicitly in \texttt{run\_experiments.py}. We verified the execution of the algorithms by producing detailed terminal logs and saving step-by-step evaluation scores to \texttt{experiment\_results\_timeseries.json}. HV is highly rigorous for this context because it directly captures both the convergence toward the global optimum and the diversity of the discovered configurations. Furthermore, to determine the practical utility of the LLM overhead, we conducted a \textbf{Zero-Shot Warm-Start Analysis}, tracking HV at every active-learning step (from $t=1$ to $t=10$) to observe exactly when and how the evolutionary LLM surpasses statistical non-LLM baselines.
...
\begin{table}[h]
\caption{Hypervolume across ten MOOT datasets after a budget of 10 evaluations (higher is better). While the Greedy baseline occasionally achieves high HV, it suffers from severe objective diversity collapse (Pareto Recall $\approx 0$).}
...
\begin{figure}[h]
  ...
  \caption{Zero-Shot Warm-Start Analysis (Average Hypervolume vs. Active-Learning Step). HSEvo maintains a distinct advantage from step $t=1$ due to the LLM's semantic prior knowledge.}
  \label{fig:warm_start}
\end{figure}

The \textbf{Zero-Shot Warm-Start Analysis} (Figure~\ref{fig:warm_start}) revealed fascinating dynamics. HSEvo achieved an average final HV of 0.793, significantly outperforming the state-of-the-art non-LLM statistical solvers MOTPE (0.742) and NSGA-II (0.726). 

Crucially, the step-by-step timeseries plot directly addresses at which active-learning step the LLM performs better. As shown in Figure~\ref{fig:warm_start}, there is no traditional "crossover" where a slower algorithm eventually overtakes another. Instead, HSEvo demonstrates a massive \emph{Zero-Shot Warm-Start Advantage}. From the very first evaluation step ($t=1$), HSEvo outperforms MOTPE and NSGA-II and maintains this lead throughout the entire budget. While MOTPE relies purely on numerical search and struggles to explore diverse regions within a tight 10-step budget, the LLM leverages semantic domain knowledge to write selection functions that instantly target optimal, high-variance regions.

\textbf{Explaining the Greedy Baseline:} The Greedy baseline appears to achieve the highest average HV (0.904), jumping to $\sim$0.9 HV within just 2 learning steps. However, this is a known artifact of highly-correlated objective landscapes and does \emph{not} mean Greedy is the best algorithm. Because Greedy perfectly optimizes a single primary objective, it instantly finds an extreme boundary point of the Pareto front, capturing a massive rectangular volume of the dominated space. However, it suffers from total \textbf{objective diversity collapse}. Its \textbf{Pareto Recall} is functionally near zero because it cannot find balanced trade-off solutions, making it practically useless for system engineers who need diverse options to balance latency and cost.

Regarding RQ2, our redefined metrics highlighted clear practical trade-offs:
\begin{itemize}
\item \textbf{Cost:} HSEvo incurred a noticeable Token Usage Overhead ($\sim$6,000 tokens per dataset over 4 evaluation cycles) and API latency ($\sim$5 minutes total wall-clock time), whereas the local statistical baselines (MOTPE, NSGA-II) executed near-instantaneously ($<0.1$ seconds).
\item \textbf{Robustness:} We calculated Algorithmic Variance as the standard deviation of final Hypervolume across all 10 datasets. HSEvo exhibited a cross-dataset standard deviation of $0.216$, which is slightly higher than MOTPE ($0.197$) and NSGA-II ($0.198$). This indicates that while HSEvo achieves a higher mean Hypervolume, the LLM's heuristic generation exhibits slightly higher variance across varied system landscapes than deterministic mathematical solvers. However, it still avoided the premature population collapse observed in older frameworks like ReEvo.
\item \textbf{Explainability:} Unlike the opaque numerical weight matrices of MOTPE, HSEvo generated fully human-readable Python functions. The resulting active-learning heuristics exhibited an average McCabe Cyclomatic Complexity of exactly $V(G) = 3.67$ (ranging from $2$ to $6$), consisting of simple, interpretable Euclidean distance calculations and readable conditional logic.
\end{itemize}
...
All scripts, output logs, JSON result files (\texttt{experiment\_results\_timeseries.json}), problem definitions, and dependencies (\texttt{requirements.txt}) required to replicate these experiments are available in our public GitHub repository at: \url{https://github.com/Akshay-Dongare/SE-AI-Group-15/tree/main/hw6}.
