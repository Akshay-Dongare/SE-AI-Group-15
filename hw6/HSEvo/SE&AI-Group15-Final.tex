...
\textbf{Secondary Metrics (Cost, Robustness, and Explainability):} To address RQ2, we define three qualitative and quantitative metrics: (1) \textbf{Cost} is measured in total API calls per generation and the corresponding wall-clock execution time. (2) \textbf{Robustness} is measured by the framework's resistance to premature population collapse and the syntactical validity rate of the generated Python code. (3) \textbf{Explainability} is evaluated qualitatively by whether the resulting coordination algorithm is human-readable, executable Python code (as opposed to opaque model weights).
