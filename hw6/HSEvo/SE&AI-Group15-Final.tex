...
\begin{itemize}
\item \textbf{Cost:} HSEvo incurred a significant Token Usage Overhead ($\sim$15,000 tokens per dataset) and API latency ($\sim$15 minutes total), whereas the local statistical baselines (MOTPE, NSGA-II) executed near-instantaneously ($<0.1$ seconds).
\item \textbf{Robustness:} HSEvo proved algorithmically robust, maintaining competitive Hypervolume across all 10 diverse datasets without the premature population collapse observed in older frameworks like ReEvo (which hardcode large tournament sizes).
\item \textbf{Explainability:} Unlike the opaque numerical weight matrices of MOTPE, HSEvo generated fully human-readable Python functions. The resulting active-learning heuristics exhibited a low McCabe Cyclomatic Complexity ($V(G) \approx 3$), consisting of simple, interpretable Euclidean distance calculations and conditional logic.
\end{itemize}
