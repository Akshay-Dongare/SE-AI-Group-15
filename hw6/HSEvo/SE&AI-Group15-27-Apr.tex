...
Random, Greedy, MOTPE, NSGA-II & Table~2 \\
...
The runtime profile has direct implications for the
\emph{practicality} of the remaining research agenda. At
53.9 seconds per dataset, a full sweep across all 120+ MOOT
datasets with 10 seeds each would require approximately
$120 \times 10 \times 53.9 = 64{,}680$ seconds ($\sim$18 
hours) at an estimated cost of \$6--\$30 in API fees---a
feasible but non-trivial investment. Scaling to larger
evolutionary populations (e.g., 50 individuals, 100
function evaluations) would multiply this by $\sim$8$\times$,
pushing a full benchmark sweep to $\sim$6 days. This
constrains realistic deployment to offline, nightly
optimization pipelines rather than interactive or real-time
system tuning---a boundary condition practitioners should
account for before adoption.
