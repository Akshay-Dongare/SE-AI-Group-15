...
\begin{itemize}
\item Proposes a framework using evolutionary LLMs to automatically generate active-learning heuristics for multi-objective software engineering optimization tasks.
...
Figure~\ref{fig:full_convergence} extends the warm-start analysis 
to the full 100-step budget, revealing long-horizon convergence 
across all five algorithms. In the early regime ($t < 30$), 
MOTPE and HSEvo track closely, with MOTPE holding a marginal 
advantage during its surrogate model burn-in phase. HSEvo 
overtakes MOTPE near $t = 30$ and maintains a consistent lead 
through $t = 100$, confirming that the Zero-Shot Warm-Start 
Advantage is not a transient artifact but a durable property 
of LLM semantic initialization. MOTPE consistently outperforms 
NSGA-II across the entire budget, and both converge toward HSEvo 
after $t = 60$ as the evaluation history grows. Random, while 
rising steadily, remains below MOTPE and HSEvo throughout. 
Greedy dominates on raw HV but, as discussed, through diversity 
collapse rather than genuine multi-objective coverage.
...
Three datasets warrant special attention. \textbf{redis} and
...
\textbf{storm} presents a distinct case: all multi-objective
solvers (MOTPE, NSGA-II, HSEvo) achieve Pareto Recall of 1.000
while Random achieves 0.000. Unlike redis and pom3a, storm's
objective landscape does support a diverse Pareto front, but
the Random baseline fails to locate any Pareto-optimal solution
within 100 evaluations due to the high dimensionality of storm's
configuration space relative to the front size. This confirms
that Random's failure on storm is a budget-exhaustion artifact
rather than a landscape degeneracy, and is resolved by any
structured selection strategy.
